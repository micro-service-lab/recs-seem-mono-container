...
// GetGroups is a handler for getting groups.
type GetGroups struct {
	Service service.ManagerInterface
}

// GetGroupsParam is a parameter for GetGroups.
...
var getGroupsParseFuncMap = map[reflect.Type]queryparam.ParserFunc{
...
func (h *GetGroups) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	parse := queryparam.NewParser(r.URL.Query())
	var param GetGroupsParam
	err := parse.ParseWithOptions(&param, queryparam.Options{
		TagName: "queryParam",
		FuncMap: getGroupsParseFuncMap,
	})
	if err != nil {
	...
	case parameter.OrganizationWithCaseDefault:
		organizations, err = h.Service.GetOrganizations(
			ctx,
			param.SearchName,
			parameter.WhereOrganizationTypeGroup,
			uuid.UUID{},
			param.Order,
			param.Pagination,
			param.Limit,
