...
func TestGetRecordTypes_ServeHTTP(t *testing.T) {
	t.Parallel()
	fd, err := factory.Generator.NewRecordTypes(10)
	...
	cases := map[string]struct {
		...
		"simple": {
			...
			want: wants{
				...
				data: store.ListResult[entity.RecordType]{
					Data: fd.ForEntity(),
				},
				errAttr: nil,
			},
	...
	mockService := &service.ManagerInterfaceMock{
		GetRecordTypesFunc: func(
			...
			switch order {
			case parameter.RecordTypeOrderMethodName:
				dd = dd.OrderByNames()
			case parameter.RecordTypeOrderMethodReverseName:
				dd = dd.OrderByReverseNames()
			case parameter.RecordTypeOrderMethodDefault:
			}
			switch pg {
			...
			return store.ListResult[entity.RecordType]{
				Data:             dd.ForEntity(),
				WithCount:        wca,
	...
	for ni, tc := range cases {
		...
		t.Run(ni, func(t *testing.T) {
			...
			r := httptest.NewRequest(http.MethodGet, "/record_types", nil)
			q := r.URL.Query()
