...
func TestGetPermissionCategories_ServeHTTP(t *testing.T) {
	t.Parallel()
	fd, err := factory.Generator.NewPermissionCategories(10)
	...
	cases := map[string]struct {
		...
		"simple": {
			...
			want: wants{
				...
				data: store.ListResult[entity.PermissionCategory]{
					Data: fd.ForEntity(),
				},
				errAttr: nil,
			},
	...
	mockService := &service.ManagerInterfaceMock{
		GetPermissionCategoriesFunc: func(
			...
			switch order {
			case parameter.PermissionCategoryOrderMethodName:
				dd = dd.OrderByNames()
			case parameter.PermissionCategoryOrderMethodReverseName:
				dd = dd.OrderByReverseNames()
			case parameter.PermissionCategoryOrderMethodDefault:
			}
			switch pg {
	...
	h := handler.GetPermissionCategories{
		Service: mockService,
	}
	for ni, tc := range cases {
		...
		t.Run(ni, func(t *testing.T) {
			...
			r := httptest.NewRequest(http.MethodGet, "/permission_categories", nil)
			q := r.URL.Query()
