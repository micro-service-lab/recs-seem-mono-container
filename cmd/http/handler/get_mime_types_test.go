...
func TestGetMimeTypes_ServeHTTP(t *testing.T) {
	t.Parallel()
	fd, err := factory.Generator.NewMimeTypes(10)
	...
	cases := map[string]struct {
		...
		"simple": {
			...
			want: wants{
				...
				data: store.ListResult[entity.MimeType]{
					Data: fd.ForEntity(),
				},
				errAttr: nil,
			},
	...
	mockService := &service.ManagerInterfaceMock{
		GetMimeTypesFunc: func(
			...
			switch order {
			case parameter.MimeTypeOrderMethodName:
				dd = dd.OrderByNames()
			case parameter.MimeTypeOrderMethodReverseName:
				dd = dd.OrderByReverseNames()
			case parameter.MimeTypeOrderMethodDefault:
			}
			switch pg {
			...
			return store.ListResult[entity.MimeType]{
				Data:             dd.ForEntity(),
				WithCount:        wca,
	...
	for ni, tc := range cases {
		...
		t.Run(ni, func(t *testing.T) {
			...
			r := httptest.NewRequest(http.MethodGet, "/mime_types", nil)
			q := r.URL.Query()
